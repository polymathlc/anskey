...
const source = (process.env.ANSKEY_LIVE_SOURCE ? readFileSync(process.env.ANSKEY_LIVE_SOURCE, 'utf8') : index).replace(/\r\n/g, '\n');
const start = source.indexOf('var AnsKeyLive = (function () {');
...
function harness(overrides = {}) {
  ...
  const opts = { stream, worksheetId: 'worksheet1', signal: controller.signal,
  ...
  function begin(extra = {}) { const result = context.AnsKeyLive.connect({ ...opts, ...extra }); result.catch(() => {}); return result; }
...
test('uses the protected Ans Key endpoint and waits for session.started before becoming ready', async () => {
  ...
  assert.equal(call[3], 'https://us-central1-mathgen--app.cloudfunctions.net/ansKeyLive');
  ...
  assert.deepEqual(call[1], { action: 'start', worksheetId: 'worksheet1', sdp: h.answer.sdp });
