...
`window._imgRouteModel` / `imageEngineDescribe` / `IMG_BADGE_MS` /
**`imageAnnounce`** (in the classic script beside `imageRouteNote`, both put
on `window`), the `model` / `engine` fields the bridge now writes onto
`window.imageLastCall`, and `#imgEngineBadge` beside the toast CSS.
**`polymathlc/cer` and `polymathlc/math` carry the same block — ship a change
to all three.**

Every generated picture raises a **green box, bottom right**: *🖼 Picture
generated by ChatGPT Images · gpt-image-2.5-flare · key on this device*, or
*Gemini · gemini-2.5-flash-image*, with *(after another route refused)* when
the first route did not answer.

- **IT IS RAISED FROM THE BRIDGE** — `window.askGeminiImage`, the ONE door
  every picture goes through — so the 🖼️ Picture card, the ✨ AI note's picture
  kind and anything added later announce themselves without being told.
- **THE TWO HALVES OF THIS FILE SHARE NOTHING BUT `window`**, so the model
  rides there: `askOpenAiImage` writes `window._imgRouteModel` just before its
  request, the bridge clears it before EVERY attempt and reads it when the key
  route resolves; the Gemini route is one model (`AI_IMAGE_MODEL`) and needs no
  note. Without the clear, a route that REFUSED would leave its id on the box
  for the route that answered.
- **ONLY A SUCCESS ANNOUNCES.** The call is inside the `try`, after the
  no-picture check; a failure leaves `imageLastCall` empty and raises nothing.
- **A FIXED GREEN, NEVER `--ink`**, its own element rather than the
  bottom-centre `#toast` (which the next message replaces), held for
  `IMG_BADGE_MS` because a model id is read rather than glanced at, and a
  repeat within the hold stacks a ×N rather than a second box. It never prints.
- `imageRouteNote` prints the same describer's line under *Pictures:* in the AI
