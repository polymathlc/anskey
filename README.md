...
inside ✨ AI notes — a small **green box in the bottom-right corner** says
which engine drew it and the exact model id: *🖼 Picture generated by ChatGPT
Images · gpt-image-2.5-flare · key on this device*, or *Gemini ·
gemini-2.5-flash-image*. When the first route refused and the next one drew,
the box says so too.

- **It is raised from the one door every picture goes through**, so anything
  that draws a picture announces itself without being told.
- **It fires only when a picture actually came back** — a failure raises no
  box — and a run of pictures from the same model stacks into one box with a
  count rather than a pile of boxes.
- **It stays long enough to read a model id** (about six seconds), it never
  prints, and the AI Engine dialog repeats the same line under *Pictures:* so
  the last picture's model can be checked after the box has gone.
