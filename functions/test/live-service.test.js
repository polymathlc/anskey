...
function harness(overrides = {}) {
  ...
  const deps = {
    auth: { async verifyIdToken(token, revoked) { calls.push(['auth', token, revoked]); return { uid: 'child', email: TEACHER_EMAIL, email_verified: true, firebase: { sign_in_provider: 'google.com' } }; } },
...
test('App Check is required and bound to the Ans Key app', async () => {
...
test('only the verified teacher identity can start or stop live assistance', async () => {
  for (const claims of [
    { email: 'student@example.com', email_verified: true },
    { email: TEACHER_EMAIL, email_verified: false },
    { email: TEACHER_EMAIL },
    { email_verified: true },
    ...
    for (const body of [undefined, { action: 'stop', sessionId: 'live-opaque-1' }]) {
      ...
      assert.equal(response.body.error.code, 'teacher_required');
    }
