...
const SESSION_COLLECTION = 'ansKeyLiveSessions';
const LIMIT_COLLECTION = 'ansKeyLiveLimits';
  ...
  async function reserve(uid, worksheetId, now, policy) {
    ...
    await db.runTransaction(async tx => {
      const [worksheet, owner, global] = await Promise.all([
        tx.get(db.collection('pdfAnnotator').doc(worksheetId)), tx.get(ownerRef), tx.get(globalRef)
      ]);
      if (!worksheet.exists || worksheet.data().ownerUid !== uid) {
        throw new LiveError(403, 'worksheet_not_owned', 'Open one of your saved worksheets before starting live assistance.');
      }
      const ownerData = owner.data() || {};
      const globalData = global.data() || {};
      if (ownerData.currentLease) throw new LiveError(409, 'lesson_already_active', 'A live session is already open on this account. End it before starting another.');
      const starts = ownerData.day === day ? Number(ownerData.starts || 0) : 0;
      if (starts >= policy.startsPerDay) throw new LiveError(429, 'daily_limit', 'You have used today\'s live sessions. You can still record your explanation.');
      ...
      if (Object.keys(active).length >= policy.concurrent || globalStarts >= policy.globalStartsPerDay) {
        throw new LiveError(429, 'live_busy', 'Live assistance is busy or has reached today\'s allowance. You can record without AI and try again later.');
      }
      active[id] = true;
